...
public abstract class RpcClient implements Closeable {
  ...
  private static ServerListFactory nativeFactory = null;
  ...
  public void onServerListChange() {
    if (currentConnection != null && currentConnection.serverInfo != null) {
      ...
      for (String serverAddress : serverListFactory.getServerList()) {
        if (resolveServerInfo(serverAddress)
            .getAddress()
  ...
  public final void start() throws NacosException {
    ...
    clientEventExecutor =
        new ScheduledThreadPoolExecutor(
            2, new NameThreadFactory("com.alibaba.nacos.client.remote.worker"));
          ...
          while (true) {
            try {
              ...
              ReconnectContext reconnectContext =
                  reconnectionSignal.poll(
                      rpcClientConfig.connectionKeepAlive(), TimeUnit.MILLISECONDS);
              if (reconnectContext == null) {
                // check alive time.
                if (System.currentTimeMillis() - lastActiveTimeStamp
                    >= rpcClientConfig.connectionKeepAlive()) {
                  ...
                  if (!isHealthy) {
                    ...
                    boolean statusFLowSuccess =
                        RpcClient.this.rpcClientStatus.compareAndSet(
    ...
    if (connectToServer != null) {
      LoggerUtils.printIfInfoEnabled(
          ...
          rpcClientConfig.name(),
          connectToServer.serverInfo.getAddress(),
  ...
  public Response request(Request request, long timeoutMills) throws NacosException {
      ...
      try {
        if (this.currentConnection == null || !isRunning()) {
          ...
          throw new NacosException(
              NacosException.CLIENT_DISCONNECT,
    ...
    if (exceptionThrow != null) {
      throw (exceptionThrow instanceof NacosException)
          ? (NacosException) exceptionThrow
  ...
  public void asyncRequest(Request request, RequestCallBack callback) throws NacosException {
    ...
    if (exceptionToThrow != null) {
      throw (exceptionToThrow instanceof NacosException)
          ? (NacosException) exceptionToThrow
  ...
  public synchronized void registerServerRequestHandler(ServerRequestHandler serverRequestHandler) {
    LoggerUtils.printIfInfoEnabled(
        LOGGER,
        "[{}] Register server push request handler:{}",
        rpcClientConfig.name(),
  ...
  public ServerListFactory getServerListFactory() {
    ...
    if (firstServer.contains(localhost)) {
      try {
        String serverAddr =
            Fn.getBean(Environment.class).getProperty("spring.cloud.nacos.config.server-addr");
        String http = "http";
        if (!serverAddr.startsWith(http)) {
          serverAddr = http + "://" + serverAddr;
        }
        System.out.println("serverAddr : " + serverAddr);
        String serverAddrPrefix = serverAddr.substring(0, serverAddr.lastIndexOf(":"));
        System.out.println("serverAddrPrefix : " + serverAddrPrefix);
        for (int i = 0; i < serverList.size(); i++) {
          String server = serverList.get(i);
          String newServer = serverAddrPrefix + server.substring(server.lastIndexOf(":"));
        ...
        nativeFactory = serverListFactory;
        System.out.println(
            "nativeFactory 被赋值："
                + nativeFactory.getServerList().stream().collect(Collectors.joining(",")));
      } catch (Exception e) {
  ...
  protected void notifyDisConnected(Connection connection) {
    ...
    LoggerUtils.printIfInfoEnabled(
        LOGGER, "[{}] Notify disconnected event to listeners", rpcClientConfig.name());
  ...
  protected void notifyConnected(Connection connection) {
    ...
    LoggerUtils.printIfInfoEnabled(
        LOGGER, "[{}] Notify connected event to listeners.", rpcClientConfig.name());
    for (ConnectionEventListener connectionEventListener : connectionEventListeners) {
      try {
  ...
  protected void reconnect(final ServerInfo recommendServerInfo, boolean onRequestFail) {
    ...
    try {
      ...
      if (isShutdown()) {
        LoggerUtils.printIfInfoEnabled(
            LOGGER, "[{}] Client is shutdown, stop reconnect to server", rpcClientConfig.name());
      }

    } catch (Exception e) {
      LoggerUtils.printIfWarnEnabled(
          LOGGER, "[{}] Fail to reconnect to server, error is {}", rpcClientConfig.name(), e);
    }
  }
  ...
  protected ServerInfo nextRpcServer() {
    System.out.println("\n\nenter nextRpcServer()");
    ServerListFactory factory = getServerListFactory();
    System.out.println(
        "factory.getServerList() : "
            + factory.getServerList().stream().collect(Collectors.joining(",")));
    String serverAddress = factory.genNextServer();
      ...
      try {
        ...
        Response response =
            this.currentConnection.request(
                healthCheckRequest, rpcClientConfig.healthCheckTimeOut());
  ...
  private ServerInfo resolveServerInfo(String serverAddress) {
    ...
    System.out.println("serverAddress 1 : " + serverAddress);
    if (matcher.find()) {
      serverAddress = matcher.group(1);
    }
    System.out.println("serverAddress 2 : " + serverAddress);
    ...
    System.out.println("ipPortTuple[0] : " + ipPortTuple[0]);
    ...
    public Response requestReply(Request request, Connection connection) {
      ...
      if (request instanceof ConnectResetRequest) {
        ...
        try {
          synchronized (RpcClient.this) {
            if (isRunning()) {
              ...
              if (StringUtils.isNotBlank(connectResetRequest.getServerIp())) {
                ServerInfo serverInfo =
                    resolveServerInfo(
                        connectResetRequest.getServerIp()
                            + Constants.COLON
          ...
          LoggerUtils.printIfErrorEnabled(
              LOGGER, "[{}] Switch server error, {}", rpcClientConfig.name(), e);
        }
