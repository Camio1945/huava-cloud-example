...
/**
 * 订单提交服务（使用全局事务，为了方便测试，下游的接口有一半的概率会抛异常）<br>
 *
 * @author Camio1945
 */
@Slf4j
...
class SubmitOrderWithGlobalTransactionService extends BaseService<OrderMapper, OrderPo> {
  ...
  private void buyGoods(List<OrderItemPo> orderItems) {
    for (OrderItemPo orderItem : orderItems) {
      ...
      goodsFeignClient.buyGoodsWithGlobalTransactionService(decreaseStockQo);
    }
  }
  ...
  private GoodsDto getGoodsPo(OrderItemPo orderItem) {
    ...
    sendMqMessageIfNeeded(orderItem, goodsPo);
    Assert.isTrue(goodsPo.getStock() >= orderItem.getGoodsCount(), "商品库存不足");
    return goodsPo;
  }

  private void sendMqMessageIfNeeded(OrderItemPo orderItem, GoodsDto goodsPo) {
